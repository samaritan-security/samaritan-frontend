...
class ActionRequest extends React.Component<IActionRequestProps, IActionRequestState> {
    state = {
        name : "",
        visible : false,
        modalVisible: false,
    ...
    render(){
        const content = (<div style={{display: "block"}}>
            <div style={{margin: 2}}>
                <Button type="primary" size="small" onClick={this.showRecognizeModal} block ghost> Recognize</Button>
            </div>
            <Modal
                title="Recognize"
            ...
            <div style={{margin: 2}}>
                <Button type="primary" size="small" onClick={(e) => this.showConfirmAuthorize(e)} block ghost> Authorize</Button>
            </div>
            <div style={{margin: 2}}>
                <Button type="danger" size="small" onClick={(e) => this.showConfirmUnathorize(e)} block ghost> Unauthorize </Button>
            </div>
         </div>)
        return(
            <>
                <Popover title="Actions" content={content} placement="top" visible={this.state.visible}>
                    <Button shape="circle" icon="ellipsis" onClick={this.handleShowActions} />
                </Popover>
            </>
        )
    }
